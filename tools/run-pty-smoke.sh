#!/usr/bin/env bash
# Headless interactive (PTY) test for the GNU bash + mlibc port.
#
# Boots InstantOS in QEMU with a PTY launcher standing in for /bin/login. The
# launcher opens /dev/ptmx, spawns bash -i on the slave, then scripts a series
# of commands to the master and forwards bash's output to serial (prefixed by
# the bytes themselves). This exercises the full kernel PTY line discipline,
# tty buffering, and job-control paths the graphics terminal relies on.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
BUILD_DIR="${BUILD_DIR:-$ROOT/build}"
OUT_DIR="${OUT_DIR:-$BUILD_DIR/pty-smoke}"
LOG="${LOG:-$OUT_DIR/pty-smoke.serial.log}"
...
if [ "$status" -ne 0 ] && [ "$status" -ne 124 ]; then
  printf 'pty smoke qemu failed with status=%s\n' "$status" >&2
  exit "$status"
fi

if grep -q 'Unknown Instruction' "$LOG" || grep -q 'User process crash' "$LOG"; then
  printf 'pty smoke: detected a user process crash in %s\n' "$LOG" >&2
  grep -nE 'Unknown Instruction|User process crash|exception:|rip:' "$LOG" >&2 || true
  exit 1
fi

fail=0
check() { if ! grep -q "$1" "$LOG"; then printf 'pty smoke: missing %q in %s\n' "$1" "$LOG" >&2; fail=1; fi; }
check 'PTYTEST] ptmx-open ok'
check 'PTYTEST] spawned pid='
check 'PTY_HELLO'      # command output
check 'val=21'         # $((X*3)) with X=7
check 'loop1'          # for-loop body
check 'loop3'
check 'PTYTEST] done'

if [ "$fail" -ne 0 ]; then
  printf '\n--- captured PTY session ---\n' >&2
  grep -nE 'PTYTEST|PTY_HELLO|val=|loop|ISH' "$LOG" >&2 || true
  exit 1
fi

printf 'pty smoke passed: %s\n' "$LOG"
grep -nE 'PTYTEST|PTY_HELLO|val=|loop|ISH' "$LOG" || true
